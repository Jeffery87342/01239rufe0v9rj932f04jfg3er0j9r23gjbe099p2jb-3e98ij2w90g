...
class RobloxAccountCreator:
    ...
    def validate_username(self, username: str) -> bool:
        ...
        try:
            ...
            self._debug_print(f"Validating username: {username}")
            ...
            if response.status_code == 200:
                ...
                if not is_valid:
                    self._debug_print(f"Username '{username}' is taken")
    ...
    def get_funcaptcha_token(self) -> Optional[str]:
        ...
        """
        self._debug_print('Solving FunCaptcha...')
        ...
        try:
            ...
            if token:
                self._debug_print('FunCaptcha solved successfully!')
                return token
            else:
    ...
    def create_account(self, username: str, password: str, birthday: Tuple[int, int, int], 
        ...
        try:
            self._debug_print(f"Creating account: {username}")
    ...
    def create_random_account(self) -> Optional[Dict]:
        ...
        self._debug_print('Generating random credentials...')
        username = self.generate_username()
        password = self.generate_password()
        birthday = self.generate_birthday()
        
        self._debug_print(f'Username: {username}')
        self._debug_print(f'Password: {password}')
        self._debug_print(f'Birthday: {birthday[1]:02d}/{birthday[0]:02d}/{birthday[2]}')
        
        # Validate username
        self._debug_print('Validating username...')
        max_attempts = 5
        attempt = 0
        
        while attempt < max_attempts:
            if self.validate_username(username):
                self._debug_print(f'Username "{username}" is available!')
                break
            else:
                if self.last_error and "RATE LIMITED" in self.last_error:
        ...
        if attempt >= max_attempts:
            self._handle_error("USERNAME ERROR", "Failed to find available username")
            return None
        ...
        # Create account
        self._debug_print('Creating account...')
        result = self.create_account(username, password, birthday, captcha_token)
        
        if result and result['status_code'] == 200:
            self._debug_print('Account created successfully!')
